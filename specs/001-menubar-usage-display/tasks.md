...
- [x] T014 [US1] [depends: T013] In `menubar_app.py`, call `usage_parser.get_summary()` and render its result into the dropdown as four top-level buckets — Today, Current Session, Last 5 Hours, All Time. Per FR-007, each bucket is itself a submenu exposing all five values (input, output, cache read, cache creation, and the combined total), all formatted with `format_tokens()`. Trigger this on app startup so the menu is populated before first click
...
- [x] T022 [US3] [depends: T020, T021] Manually run quickstart.md V4 (missing log directory), V5 (malformed input survivability), and the Install section end-to-end on a fresh clone; confirm no undocumented steps were needed, and re-confirm no Dock icon or Cmd+Tab entry appears on this fresh install
  > **Caveat (resolved)**: at the time this first ran, nothing was committed to git yet, so "fresh clone" was simulated via an rsync copy of the working tree, not a real `git clone`. Re-validated afterward against an actual `git clone` of `origin/001-menubar-usage-display` into a clean temp directory: clone (~2s) + venv + `pip install -r requirements.txt` (~3s) all succeeded with no undocumented steps; the app ran with `NSApplicationActivationPolicyAccessory` confirmed (no Dock icon/Cmd+Tab entry); V4 (missing log directory) returned `0 0 None`; V5 (malformed input) returned `entries: 0 skipped: 3`; `pytest -q` (13 tests) and `ruff check .` both passed after installing `requirements-dev.txt`. SC-001's install budget is met by a wide margin (~5s total vs. the 5-minute target).
